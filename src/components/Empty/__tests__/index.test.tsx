...
const IMAGE_zxuicn = '//gw.alicdn.com/tfs/TB1uYH4QoY1gK0jSZFMXXaWcVXa-218-56.svg';
...
describe('<Empty />', () => {
  ...
  it('should render image (custom)', () => {
    const { container } = render(<Empty image={IMAGE_zxuicn} />);
    const element = container.querySelector('img');

    expect(element?.getAttribute('src')).toBe(IMAGE_zxuicn);
  });
});
